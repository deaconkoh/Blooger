...
function search_posts_by_title($query) {
    global $conn;
    // Prepare the query to search for titles that start with the search query, case-insensitive
    $stmt = $conn->prepare("
        SELECT p.*, u.username 
        FROM post_info p 
        JOIN user_info u ON p.owner_id = u.member_id 
        ...
        ORDER BY p.created_at DESC
    ");
    $search_query = $query . '%';  // Use the '%' wildcard to match titles starting with the query
